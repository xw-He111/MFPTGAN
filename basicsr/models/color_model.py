...
class ColorModel(BaseModel):
    ...
    def feed_data(self, data):
        self.pan_l = data['pan_l'].to(self.device)
        self.pan_ab = data['pan_ab'].to(self.device)
        self.pan_rgb = tensor_lab2rgb(torch.cat([self.pan_l, self.pan_ab], dim=1))

        self.label_ab = data['label_ab'].to(self.device)
        self.label_l = data['label_l'].to(self.device)
        self.label_lab = torch.cat([self.label_l, self.label_ab], dim=1)
        self.label_rgb = tensor_lab2rgb(self.label_lab)

        if self.opt['train'].get('color_enhance', False):
            for i in range(self.label_rgb.shape[0]):
                self.label_rgb[i] = color_enhacne_blend(self.label_rgb[i], factor=self.opt['train'].get('color_enhance_factor'))
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output_ab, self.output_l = self.net_g(self.pan_rgb)
        self.output_lab = torch.cat([self.output_l, self.output_ab], dim=1)
        ...
        # pixel loss
        if self.cri_pix:
            l_g_pix_ab = self.cri_pix(self.output_ab, self.label_ab)
            l_g_pix_l = self.cri_pix(self.output_l, self.label_l)

            l_g_total += (l_g_pix_ab + l_g_pix_l)

            loss_dict['l_g_pix'] = l_g_pix_l + l_g_pix_ab

        # perceptual loss
        if self.cri_perceptual:
            l_g_percep, l_g_style = self.cri_perceptual(self.output_rgb, self.label_rgb)
        ...
        real_d_pred = self.net_d(self.label_rgb)
    ...
    def get_current_visuals(self):
        ...
        out_dict['pan'] = self.pan_rgb.detach().cpu()
        out_dict['result'] = self.output_rgb.detach().cpu()
        if self.opt['logger'].get('save_snapshot_verbose', False):  # only for verbose
            self.output_lab_chroma = torch.cat([torch.ones_like(self.output_l) * 50, self.output_ab], dim=1)
            self.output_rgb_chroma = tensor_lab2rgb(self.output_lab_chroma)
            out_dict['result_chroma'] = self.output_rgb_chroma.detach().cpu()

        # if hasattr(self, 'gt'):
        out_dict['label'] = self.label_rgb.detach().cpu()
        if self.opt['logger'].get('save_snapshot_verbose', False):  # only for verbose
            self.label_lab_chroma = torch.cat([torch.ones_like(self.label_l) * 50, self.label_ab], dim=1)
            self.label_rgb_chroma = tensor_lab2rgb(self.label_lab_chroma)
            out_dict['gt_chroma'] = self.label_rgb_chroma.detach().cpu()
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output_ab, self.output_l = self.net_g_ema(self.pan_rgb)
                self.output_lab = torch.cat([self.output_l, self.output_ab], dim=1)
                self.output_rgb = tensor_lab2rgb(self.output_lab)
        else:
            self.net_g.eval()
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            # if idx == 100:
            #     break
            img_name = osp.splitext(osp.basename(val_data['pan_path'][0]))[0]
            ...
            if 'label' in visuals:
                gt_img = tensor2img([visuals['label']])
            ...
            if with_metrics:
                ...
                for name, opt_ in self.opt['val']['metrics'].items():
                    if name == 'fid':
                        pred, gt = visuals['result'].cuda(), visuals['label'].cuda()

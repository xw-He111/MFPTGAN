...
from basicsr.archs.tdcolor_arch_utils.unet import Hook, CustomPixelShuffle_ICNR,  UnetBlockWide, NormType, custom_conv_layer
from basicsr.archs.tdcolor_arch_utils.convnext import ConvNeXt
from basicsr.archs.tdcolor_arch_utils.transformer_utils import SelfAttentionLayer, CrossAttentionLayer, FFNLayer, MLP
from basicsr.archs.tdcolor_arch_utils.position_encoding import PositionEmbeddingSine
from basicsr.archs.tdcolor_arch_utils.transformer import Transformer
...
class TDColor(nn.Module):
    ...
    def __init__(self,
        ...
        self.refine_net_l = nn.Sequential(custom_conv_layer(100 + 3, 1, ks=1, use_activ=False, norm_type=NormType.Spectral))
    ...
    def forward(self, x):
        ...
        self.encoder(x)
        out_ab, out_l = self.decoder()

        feature_visualization_single(out_l, features_num=out_l.shape[1], 
                                     save_dir='./results_dot/3259/L')
        feature_visualization_single(out_ab, features_num=out_ab.shape[1], 
                                     save_dir='./results_dot/3259/ab')

        coarse_input_ab = torch.cat([out_ab, x], dim=1)

        coarse_input_l = torch.cat([out_l, x], dim=1)

        out_ab = self.refine_net(coarse_input_ab)

        out_l = self.refine_net_l(coarse_input_l)
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.l_decoder = MultiScaleColorDecoder_L(
            in_channels=[512, 512, 256],
            num_queries=100,
            num_scales=num_scales,
            dec_layers=dec_layers,
    ...
    def forward(self):
        ...
        out_ab = self.color_decoder([out0, out1, out2], out3)

        out_l = self.l_decoder([out0, out1, out2], out3)
    ...
    def make_layers(self):
        ...
        e_in_c = self.hooks[-1].feature.shape[1]  # 找到名字为self.hooks[-1]的编码器层的特征图的维度
        in_c = e_in_c
...
class Encoder(nn.Module):
    ...
    def __init__(self, encoder_name, hook_names, from_pretrain, **kwargs):
        ...
        self.hooks = self.setup_hooks()  # 找到四个连接后面的特征层
    ...
    def forward(self, x):
        return self.arch(x)   # 只返回四个连接后面的特征层
...
class MultiScaleColorDecoder_L(nn.Module):
...
def feature_visualization_single(features, features_num=100, save_dir='./results_dot/4123'):
    ...
    blocks = torch.chunk(features, min(features.shape[1], features_num), dim=1)

    plt.figure()
    for i in range(features_num):
        feature = blocks[i].detach().cpu().squeeze(0)
        ...
        feature = (feature * 255.0).round().astype(np.uint8)
...
def feature_visualization(features, features_num=100, save_dir='features/'):
    ...
    # 确保不超过实际可用的特征数量
    features_num = min(features_num, features.shape[1])
    blocks = torch.chunk(features, features.shape[1], dim=1)

    # 计算合适的网格布局
    rows = int(math.sqrt(features_num))
    cols = math.ceil(features_num / rows)
    
    plt.figure(figsize=(cols * 2, rows * 2))  # 根据特征数量调整图形大小
    
    for i in range(features_num):
        # 正确处理特征张量
        feature_tensor = blocks[i].squeeze().detach().cpu()
        
        # 转换为PIL图像
        feature_img = transforms.ToPILImage()(feature_tensor)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'{features_num}_{exist_img_num}.png'), 
               dpi=100, bbox_inches='tight', pad_inches=0.0)
    plt.close()
...
class ConvNeXt(nn.Module):
    ...
    Args:
        ...
        drop_path_rate (float): Stochastic depth rate. Default: 0.  随机深度率
        layer_scale_init_value (float): Init value for Layer Scale. Default: 1e-6.
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        for i in out_indices:
            layer = LayerNorm(dims[i], eps=1e-6, data_format="channels_first")  # dims=[96, 192, 384, 768]
            # layer = nn.Identity()
            layer_name = f'norm{i}'
    ...
    def forward_features(self, x):  # 输出四个层的特征图，用于Color Decoder
        for i in range(4):

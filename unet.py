...
class CustomPixelShuffle_ICNR(nn.Module):  # 图像超分辨率上采样
    ...
    def __init__(self,
        ...
        self.shuf = nn.PixelShuffle(scale)  # 将输入张量的空间分辨率（高度和宽度）按scale倍放大
        self.do_blur = blur
...
#         s = self.hook.feature  # 这一层的feature
...
class UnetBlockWide(nn.Module):
    ...
    def __init__(self,
        ...
        ni = up_out + x_in_c
        # self.conv = custom_conv_layer(ni, n_out, norm_type=norm_type, self_attention=self_attention, extra_bn=True)
        self.conv = custom_conv_layer(n_out, n_out, norm_type=norm_type, self_attention=self_attention, extra_bn=True)
        self.relu = nn.ReLU()

        self.fsa = FSA(x_in_c, n_out, n_out)
    ...
    def forward(self, up_in):
        ...
        # return self.conv(cat_x)
        s_x = self.bn(s)
        out = self.fsa(s_x, up_out)
        return self.conv(self.relu(out))
    
class FSA(nn.Module):
    def __init__(self, ic1, ic2, oc):
        super().__init__()
        self.conv_in1 = nn.Conv2d(in_channels=ic1, out_channels=oc, kernel_size=1, stride=1)
        self.conv_in2 = nn.Conv2d(in_channels=ic2, out_channels=oc, kernel_size=1, stride=1)
        self.conv_cat = nn.Conv2d(in_channels=(ic1 + ic2), out_channels=oc, kernel_size=1, stride=1)
        self.relu_1 = nn.ReLU(inplace=True)
        self.relu_2 = nn.ReLU(inplace=True)
        self.relu_cat = nn.ReLU(inplace=True)

        # self.sa_7 = SpatialAttention(kernel_size=7)
        # self.sa_5 = SpatialAttention(kernel_size=5)
        self.sa_3 = SpatialAttention(kernel_size=3)
        # self.sa_1 = SpatialAttention(kernel_size=1)

    def forward(self, input1, input2):
        x1 = self.conv_in1(input1)
        x1 = self.relu_1(x1)
        x2 = self.conv_in2(input2)
        x2 = self.relu_2(x2)
        x3 = self.conv_cat(torch.cat([input1, input2], dim=1))
        x3 = self.relu_cat(x3)

        x4 = x1 + x2 + x3
        x5_3 = self.sa_3(x4) * x4
        x5 = x5_3
...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        # nn.init.kaiming_normal_(self.conv1.weight, 0)
        # nn.init.kaiming_normal_(self.conv1.bias, 0)
